<div id="languages">
    ...
    if (!isset($writable)) {
        ?>
        <a href="javascript:void(0);" data-toggle="modal" data-target="#addLanguage" class="btn btn-primary btn-xs pull-right" style="margin-bottom:10px;"><b>+</b> <?= lang('add_lang') ?></a>
        <?php
    }
    if ($languages) {
        ?>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <?php if (MY_DEFAULT_LANGUAGE_ABBR != $language->abbr) { ?>
                            <a href="<?= base_url('admin/languages/?delete=' . $language->id) ?>" class="btn btn-danger btn-xs"><span class="glyphicon glyphicon-remove"></span> <?= lang('delete') ?></a>
                        <?php } else { ?>
                            <?= lang('is_default') ?>
                        <?php } ?>
                        <a href="<?= base_url('admin/languages/?editLang=' . $language->name) ?>" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-pencil"></span> <?= lang('edit') ?></a>
                </tr>
            <?php } ?>
        </table>
    <?php } else { ?>
        <div class="clearfix"></div><hr>
        <div class="alert alert-info"><?= lang('no_langs') ?></div>
    <?php } ?> 
    <?php
    if (isset($_GET['editLang'])) {
        ...
        <form method="POST" id="saveLang">
            ...
            <div class="alert alert-info"><span class="glyphicon glyphicon-alert"></span> <?= lang('edit_lang') ?> <b><?= ucfirst($_GET['editLang']) ?></b></div>
            <?php
            $o = 1;
            ...
            <?php } else { ?>
                <a href="javascript:void(0);" data-form-id="saveLang" style="margin-left: 10px;" class="btn btn-lg btn-info confirm-save"><?= lang('save') ?></a>
            <?php } ?>
            <a href="<?= base_url('admin/languages') ?>" class="btn btn-lg btn-default"><?= lang('cancel') ?></a>
        </form>
        <?php
    }
    ?>
            ...
            <div class="modal-content">
                <form action="" method="POST" enctype="multipart/form-data">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="myModalLabel"><?= lang('add_lang') ?></h4>
                    </div>
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="abbr"><?= lang('abbriviation') ?></label>
                        ...
                        <div class="form-group">
                            <label class="control-label"><?= lang('currency_key') ?></label>
