...
class Texts extends ADMIN_Controller
{
    ...
    public function index($page = 0)
    {
        ...
        if (isset($_POST['value'])) {
            $result = $this->Admin_model->setValueStore($_POST['my_key'], $_POST['value'], $_POST['update']);
            if ($result == true) {
        ...
        if (isset($_GET['edit'])) {
            $result = $this->Admin_model->getOneValueStore($_GET['edit']);
            $data['my_key'] = $result['my_key'];
            $data['value'] = $result['value'];
        }

        if (isset($_GET['delete'])) {
            $this->Admin_model->deleteValueStore($_GET['delete']);
            redirect('admin/texts');
        }

        $rowscount = $this->Admin_model->valueStoresCount();
        $data['valueStores'] = $this->Admin_model->getValueStores($this->num_rows, $page);
        $data['links_pagination'] = pagination('admin/texts', $rowscount, $this->num_rows, 3);

        $this->load->view('_parts/header', $head);
        $this->load->view('publisher/texts', $data);
