...
class Articles extends ADMIN_Controller
{
    ...
    public function index($page = 0)
    {
        ...
        if (isset($_GET['delete'])) {
            $result = $this->AdminModel->deleteArticle($_GET['delete']);
            if ($result == true) {
        ...
        $rowscount = $this->AdminModel->articlesCount($search);
        $data['articles'] = $this->AdminModel->getArticles($this->num_rows, $page, $search, $orderby);
        ...
        $this->load->view('newspaper/articles', $data);
    ...
    {
        $this->login_check();
        $result = $this->AdminModel->articleStatusChange($_POST['id'], $_POST['to_status']);
        if ($result == true) {
            echo 1;
        } else {
            echo 0;
